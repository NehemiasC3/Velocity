...
class ContractController {
    ...
    static async getContracts(req, res) {
        try {
            ...
            const [total, data] = await Promise.all([
                db_1.prisma.contract.count({ where }),
    ...
    static async getContractDetails(req, res) {
        try {
            ...
            const contract = await db_1.prisma.contract.findUnique({
                where: { id },
                select: {
                    id: true,
                    contractNumber: true,
    ...
    static async createContract(req, res) {
        try {
            ...
            const newContract = await db_1.prisma.$transaction(async (tx) => {
                ...
                if (cleanIp) {
                    ...
                    if (conflictingContract) {
                        throw new Error(`IP_CONFLICT: La IP ${cleanIp} ya está asignada al contrato ${conflictingContract.contractNumber || conflictingContract.id} (${conflictingContract.client?.name || 'Otro cliente'}).`);
                    }
                }
    ...
    static async updateContract(req, res) {
        try {
            ...
            const existingContract = await db_1.prisma.contract.findUnique({
                where: { id }
            });
            if (!existingContract) {
                res.status(404).json({
            ...
            const updated = await db_1.prisma.$transaction(async (tx) => {
                ...
                if (servicePlanId !== undefined)
                    updateData.servicePlanId = servicePlanId || null;
                if (routerServerId !== undefined)
                    updateData.routerServerId = routerServerId || null;
                if (cleanIp !== undefined)
                    updateData.ipAddress = cleanIp;
                if (status !== undefined)
                    updateData.status = status.toString().trim().toUpperCase();
                if (req.body.origin !== undefined)
                    updateData.origin = req.body.origin.toString().trim().toUpperCase();
    ...
    static async getPlans(_req, res) {
        try {
            let plans = await db_1.prisma.servicePlan.findMany({
            ...
            if (plans.length === 0) {
                ...
                await db_1.prisma.servicePlan.createMany({
                    data: defaultPlans
                });
                plans = await db_1.prisma.servicePlan.findMany({
                    orderBy: { downloadSpeed: 'asc' },
                    select: {
                        id: true,
                        name: true,
