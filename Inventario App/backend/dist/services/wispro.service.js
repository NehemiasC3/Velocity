...
class WisproService {
    ...
    static async getLocalContracts(options) {
        ...
        const velocityWhere = { origin: 'VELOCITY' };
        if (options?.search && options.search.trim()) {
            const q = options.search.trim();
            velocityWhere.OR = [
        ...
        const [rows, totalWispro, config, totalAll, withSerialCount, withNapCount, enabledCount, velocityContracts, velocityTotal] = await Promise.all([
            ...
            db_1.prisma.contract.count({ where: velocityWhere })
        ]);
        // Mapeo contratos nativos Velocity
        const velocityMapped = velocityContracts.map(vc => ({
            id: vc.id,
