...
import { SerializedStatus, AuditEventType, Role } from '@prisma/client';
import { inventoryService } from '../services/inventory.service';

/**
 * Resuelve el usuario responsable para auditoría y asignaciones.
 */
async function resolveResponsibleUser(req: Request): Promise<any> {
  ...
  const admin = await prisma.user.findFirst({ where: { role: Role.SUPERADMIN } }) || await prisma.user.findFirst();
...
export class ClientAssignmentController {
  ...
  static async createAssignment(req: Request, res: Response): Promise<void> {
    try {
      ...
      const itemIds: string[] = Array.isArray(rawItems) ? rawItems.map(String) : [];
      ...
      let unifiedContract = await prisma.contract.findFirst({
        where: {
          OR: [
            { id: targetIdentifier },
      ...
      if (!unifiedContract) {
        const wClient = await prisma.wisproClient.findFirst({
          where: {
            OR: [
        ...
        if (wClient) {
          ...
          try {
            ...
            let bridgeClient = await prisma.client.findFirst({
              where: {
                OR: [
            ...
            if (!bridgeClient) {
              bridgeClient = await prisma.client.create({
                data: {
                  name: wClient.name,
            ...
            unifiedContract = await prisma.contract.create({
              data: {
      ...
      if (!targetNodeId) {
        const userNode = (req as any).user?.assignedNodeId;
        if (userNode) {
          targetNodeId = userNode;
        } else {
          const firstItem = await prisma.serializedItem.findUnique({
            where: { id: itemIds[0] },
      ...
      const itemsToAssign = await prisma.serializedItem.findMany({
        where: { id: { in: itemIds } },
      ...
      const unavailable = itemsToAssign.filter(
        i => i.status === SerializedStatus.INSTALADO_CLIENTE || i.status === SerializedStatus.EN_TRANSITO
      );
      ...
      await inventoryService.invalidateDashboardCache();
   ...
   */
  static async getContractAssignment(req: Request, res: Response): Promise<void> {
    try {
      ...
      const assignedItems = await prisma.serializedItem.findMany({
        where: {
          OR: [
            { contractId: cleanContractId, status: SerializedStatus.INSTALADO_CLIENTE },
            { installedContractId: cleanContractId, status: SerializedStatus.INSTALADO_CLIENTE },
   ...
   */
  static async unassignItem(req: Request, res: Response): Promise<void> {
    try {
      ...
      const item = await prisma.serializedItem.findUnique({
        where: { id: itemId },
      ...
      const destinationWarehouseId: string = returnWarehouseId || item.clientAssignment?.nodeId || item.currentWarehouseId;
   ...
   */
  static async searchAvailableItems(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (category && category !== 'ALL') {
        where.product = { ...where.product, category: category as any };
      }
...
export const AssignmentController = ClientAssignmentController;
