...
export class LiquidationController {
   ...
   */
  public static async getLiquidations(req: Request, res: Response): Promise<void> {
    try {
   ...
   */
  public static async consumeLiquidation(req: Request, res: Response): Promise<void> {
    try {
      const {
        vehicleWarehouseId,
        ...
        retiredDeviceMac,
        retiredDeviceStatus,
        batchedUsage, // { batchId, batchNumber, metersUsed }
      ...
      if (installedOnuMac) {
        ...
        targetOnu = await prisma.serializedItem.findFirst({
          where: {
            macAddress: cleanMac,
      ...
      const sanitizedBulkUsage: { productId: string; quantity: number }[] = [];
      ...
      const result = await prisma.$transaction(async (tx) => {
        ...
        if (targetBatch && metersToDeduct > 0) {
          ...
          await tx.batchItem.update({
            ...
            data: {
              ...
              status: isAgotado ? BatchStatus.AGOTADO : BatchStatus.DISPONIBLE
            }
          });
        ...
        const ticket = await tx.installationTicket.create({
          data: {
            ...
            type: ticketType as InstallationTicketType,
