...
import { WarehouseType, SerializedStatus, WisproClientStatus, AuditEventType, Role } from '@prisma/client';
...
export class WisproService {
   ...
   */
  private static normalizeMac(mac?: string | null): string {
    if (!mac) return '';
   ...
   */
  public static async fetchActiveContracts(options?: {
    page?: number;
    perPage?: number;
    loadAll?: boolean;
    forceRefresh?: boolean;
  }): Promise<{ contracts: WisproContract[]; total: number; page: number; perPage: number; totalPages: number } & WisproContract[]> {
    const now = Date.now();
        ...
        if (firstRes && firstRes.status !== 401) {
          ...
          if (shouldLoadAll && totalPages > 1) {
            ...
            for (let i = 0; i < remainingPages.length; i += 10) {
              ...
              const batchResults = await Promise.all(
                batch.map(p => 
                  this.request<any>(`/contracts?filter%5Bstate%5D=enabled&per_page=100&page=${p}`)
                    .then(res => (res && res.data ? res.data : []))
                    .catch(() => [])
                )
              );
              batchResults.forEach(data => {
                if (Array.isArray(data)) remoteContracts.push(...data);
              });
            }
    ...
    let normalized: WisproContract[] = remoteContracts.map(r => this.normalizeContract(r));
  ...
  public static async reconcileMobileEquipment(targetContracts?: WisproContract[]): Promise<{
    ...
    for (const contract of contracts) {
      ...
      const matchedItem = availableMobileItems.find(item => {
        ...
        if (contractNormMac && itemNormMac && itemNormMac === contractNormMac) return true;
        if (contractSerial && itemSerial && itemSerial === contractSerial) return true;
        return false;
      });
  ...
  public static async fetchOpenTickets(): Promise<any[]> {
    ...
    const technicians = await prisma.user.findMany({
      where: {
        role: { in: [Role.TECNICO, Role.SUPERVISOR_MESA, Role.SUPERADMIN] }
      },
      include: {
        managedWarehouses: {
    ...
    if (wisproApiKey) {
      try {
        const [issuesRes, jobsRes]: any[] = await Promise.allSettled([
