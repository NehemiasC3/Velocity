...
export const LiquidationModal: React.FC<LiquidationModalProps> = ({
  ...
  return (
    <div 
      className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 overflow-hidden bg-transparent"
      onClick={onClose}
    >
      <div 
        className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl sm:rounded-3xl max-w-4xl xl:max-w-5xl w-full shadow-2xl ring-1 ring-slate-900/10 flex flex-col max-h-[94vh] sm:max-h-[90vh] overflow-hidden animate-in fade-in zoom-in-95 duration-150"
        onClick={(e) => e.stopPropagation()}
      >
        
        {/* Header - Fijo arriba, nunca se corta */}
        <div className="flex items-center justify-between p-4 sm:p-5 pb-3 sm:pb-4 border-b border-slate-100 dark:border-slate-800 shrink-0 bg-white dark:bg-slate-900">
          <div className="flex items-center gap-2.5">
            <div className="p-2 sm:p-2.5 rounded-xl bg-emerald-100 text-emerald-700 dark:bg-emerald-950/60 dark:text-emerald-300">
        ...
        {/* Feedback Alert si existe (fijo arriba) */}
        {feedback && (
        ...
        <form onSubmit={handleSubmit} className="flex-1 flex flex-col min-h-0 overflow-hidden text-xs">
          ...
          {/* Cuerpo con Scroll Interno fluido y Layout Responsivo en 2 Columnas para Desktop/Laptops */}
          <div className="flex-1 overflow-y-auto p-4 sm:p-5">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-3.5 sm:gap-4 items-start">
              
              {/* ── COLUMNA 1 (IZQUIERDA): VEHÍCULO, DATOS DEL TRABAJO Y ONU ── */}
              <div className="space-y-3">
                {/* SECCIÓN 1: VEHÍCULO Y DATOS DEL TRABAJO */}
                <div className="p-3.5 bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-700 rounded-2xl space-y-2.5">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                    <div>
                      ...
                      <select
                        ...
                        className="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-xl px-2.5 py-1.5 text-slate-900 dark:text-white font-bold outline-none focus:ring-2 focus:ring-emerald-500"
                      >
                        {vehicles.map(v => (
                    ...
                    <div>
                      ...
                      <select
                        ...
                        className="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-xl px-2.5 py-1.5 text-slate-900 dark:text-white font-semibold outline-none focus:ring-2 focus:ring-emerald-500"
                      >
                        <option value="INSTALACION_NUEVA">🟢 Instalación Nueva</option>
                        <option value="CAMBIO_EQUIPO">🟡 Cambio de Equipo / ONU</option>
                        <option value="REPARACION_DROP">🔵 Reparación / Empalme</option>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-xl px-2.5 py-1.5 text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-emerald-500"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-xl px-2.5 py-1.5 text-slate-900 dark:text-white font-mono outline-none focus:ring-2 focus:ring-emerald-500"
                      />
                    </div>
                  </div>
                </div>

                {/* SECCIÓN 2: ONU / ROUTER SERIADO */}
                <div className="p-3.5 bg-blue-50/50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-900/60 rounded-2xl space-y-2">
                  ...
                  <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                    <div className="relative flex-1">
                      <QrCode className="w-4 h-4 text-slate-400 absolute left-3 top-2" />
                      <input
                        type="text"
                        placeholder="Escanear o escribir MAC..."
                        value={scannedMac}
                        onChange={(e) => {
                          setScannedMac(e.target.value);
                          setSelectedOnuId('');
                        }}
                        className="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl pl-9 pr-3 py-1.5 text-xs font-mono uppercase text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-blue-500"
                      />
                    </div>
                    ...
                    <select
                      ...
                      className="w-full sm:w-44 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl px-2 py-1.5 text-xs text-slate-700 dark:text-slate-300 outline-none"
                    >
                    ...
                    <div>
                      {matchedOnu ? (
                        <span className="inline-flex items-center gap-1.5 text-[11px] font-bold text-emerald-700 dark:text-emerald-300 bg-emerald-100 dark:bg-emerald-950/60 px-2.5 py-1 rounded-lg">
                          ...
                          <span>Validada: {matchedOnu.product?.name || matchedOnu.brand} ({matchedOnu.serialNumber})</span>
                        </span>
                      ) : (
                        <span className="inline-flex items-center gap-1.5 text-[11px] font-bold text-rose-700 dark:text-rose-300 bg-rose-100 dark:bg-rose-950/60 px-2.5 py-1 rounded-lg">
                          ...
                          <span>Atención: MAC no encontrada en el vehículo</span>
                        </span>
                      )}
                    </div>
                  )}
                ...
                <div className="p-3 bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-700 rounded-2xl space-y-1">
                  ...
                  <input
                    type="text"
                    placeholder="Ej. Potencia -19.4 dBm en NAP-04 Pto 3. Conforme."
                    value={notes}
                    onChange={(e) => setNotes(e.target.value)}
                    className="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl px-2.5 py-1.5 text-xs text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-emerald-500"
                  />
                </div>
              </div>

              {/* ── COLUMNA 2 (DERECHA): CABLE DROP Y MATERIAL A GRANEL ── */}
              <div className="space-y-3">
                {/* SECCIÓN 3: BOBINA DE CABLE DROP */}
                <div className="p-3.5 bg-amber-50/50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/60 rounded-2xl space-y-2">
                  <div className="flex items-center justify-between">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                    <div>
                      ...
                      <select
                        ...
                        className="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl px-2.5 py-1.5 text-xs font-bold text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-amber-500"
                      >
                        <option value="">-- Sin consumo de cable --</option>
                        {vehicleStock.batchItems.map(b => (
                          <option key={b.id} value={b.id}>
                            {b.batchNumber} &bull; Disp: {b.currentQuantity}m
                          </option>
                        ))}
                      </select>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          className="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl px-2.5 py-1.5 text-xs font-mono font-bold text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-amber-500"
                        />
                ...
                <div className="p-3.5 bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-700 rounded-2xl space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    {loadingStock && <RefreshCw className="w-3 h-3 animate-spin text-emerald-500" />}
                  </div>

                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 pt-0.5">
                    {/* Conectores */}
                    <div className="bg-white dark:bg-slate-900 p-2 rounded-xl border border-slate-200 dark:border-slate-700 flex flex-col justify-between">
                      <span className="text-[10px] font-bold text-slate-700 dark:text-slate-300 truncate">Conectores SC</span>
                      <div className="flex items-center justify-between mt-1.5">
                        <button
                          type="button"
                          onClick={() => setConnectorsUsed(Math.max(0, connectorsUsed - 1))}
                          className="w-5 h-5 rounded bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-600 dark:text-slate-300 hover:bg-slate-200"
                        >
                          <Minus className="w-3 h-3" />
                        </button>
                        <span className="font-mono font-bold text-xs text-slate-900 dark:text-white">{connectorsUsed}</span>
                        <button
                          type="button"
                    ...
                    {/* Tensores */}
                    <div className="bg-white dark:bg-slate-900 p-2 rounded-xl border border-slate-200 dark:border-slate-700 flex flex-col justify-between">
                      <span className="text-[10px] font-bold text-slate-700 dark:text-slate-300 truncate">Tensores Drop</span>
                      <div className="flex items-center justify-between mt-1.5">
                        <button
                          type="button"
                        ...
                        <span className="font-mono font-bold text-xs text-slate-900 dark:text-white">{tensorsUsed}</span>
                        <button
                          type="button"
                    ...
                    {/* Otros materiales dinámicos */}
                    {vehicleStock.bulkStocks.slice(0, 2).map(bulk => {
                      const currentVal = bulkQuantities[bulk.productId] || 0;
                      return (
                        <div key={bulk.id} className="bg-white dark:bg-slate-900 p-2 rounded-xl border border-slate-200 dark:border-slate-700 flex flex-col justify-between">
                          ...
                          <div className="flex items-center justify-between mt-1.5">
                            ...
                            <span className="font-mono font-bold text-xs text-slate-900 dark:text-white">{currentVal}</span>
                            <button
                              type="button"
          ...
          {/* Modal Actions - Fijo al fondo */}
          <div className="flex items-center justify-end gap-2.5 px-5 py-3.5 border-t border-slate-100 dark:border-slate-800 shrink-0 bg-slate-50/90 dark:bg-slate-900/90">
            <button
              type="button"
              onClick={onClose}
