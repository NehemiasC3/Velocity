...
class WisproService {
    ...
    constructor(apiUrl, apiToken, cacheTtlMs = 5 * 60 * 1000 // 5 minutos de TTL
    ) {
        this.baseUrl = (apiUrl ||
            process.env.WISPRO_API_URL ||
            process.env.WISPRO_BASE_URL ||
            'https://www.cloud.wispro.co/api/v1').replace(/\/+$/, '');
    ...
    startWarmCacheWorker() {
        ...
        this.autoRefreshTimer = setInterval(() => {
            ...
            this.getInventory(true).catch((err) => console.warn('[WisproService] Error en auto-refresco en segundo plano:', err.message));
        }, refreshInterval);
    }
    /**
    ...
    async fetchPage(endpoint, page, perPage) {
        try {
    ...
    mapToInventoryItem(raw) {
        ...
        const mac = (raw.mac ||
            ...
            raw.device_mac ||
            'No registrada').trim();
        const serialNumber = (raw.serial_number ||
            raw.serial ||
            raw.sn ||
            raw.onu_sn ||
            raw.gpon_sn ||
            'S/N no disponible').trim();
        const model = (raw.model ||
            raw.equipment_model ||
            raw.model_name ||
            raw.hardware_model ||
            raw.device_model ||
            'Genérico / Desconocido').trim();
